...
```{r}
scented_candles_plot <- scented_data %>% 
  filter(Date >= "2019-01-01") %>% 
  mutate(month = lubridate::floor_date(Date, "month"),
         str_unscented_review = str_unscented(Review)) %>% 
  group_by(as.Date(month), unscented_review) %>% 
  ggplot(as.Date(month), unscented_review) +
  geom_bar(color = "blue",
           fill = "blue") +
  scale_x_date("Date", date_labels = "%m-%Y", date_breaks = "month") +
  scale_y_continuous(name = "Unscented Reviews") 

# <!-- When I try to run the code it gives me this error: Problem with `mutate()` input `unscented_review`. x could not find function "str_unscented" ℹ Input `unscented_review` is `str_unscented(Review)
```
