...
Av_green_data_pomp <- green_data_pomp %>% 
  rowwise() %>% 
  mutate(green_av_pomp = sum(c_across(green1:green5), na.rm = TRUE),
         comp_av_pomp = sum(c_across(comp1:comp10r), na.rm = TRUE),
         intel_av_pomp = sum(c_across(intel1:intel10r), na.rm = TRUE),
         open_tav_pomp = sum(c_across(open1:open10r), na.rm = TRUE))
```
